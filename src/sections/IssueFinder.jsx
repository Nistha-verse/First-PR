...
export default function IssueFinder({ onPractice }) {
  ...
  return (
    <section className="space-y-4 rounded-xl border border-magic/30 bg-[#0f0b1a]/80 p-5 shadow-[0_0_18px_rgba(109,40,217,0.16)]">
      <h2 className="magic-heading text-xl font-semibold">Issue Finder</h2>
      <div className="flex gap-3">
        <input value={skills} onChange={(e) => setSkills(e.target.value)} className="flex-1 rounded-md border border-magic/40 bg-black/25 p-2 text-sm" />
        <button onClick={fetchIssues} className="rounded-md bg-accent px-4 py-2 text-violet-100">{loading ? <span className="wand-tip-loader" /> : 'Find issues'}</button>
      </div>
      {error ? <p className="text-sm text-error">{error}</p> : null}
      <div className="space-y-3">
        {issues.map((issue) => (
          <article key={issue.url} className="rounded-lg border border-magic/20 bg-black/20 p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button onClick={() => explain(issue)} className="rounded border border-magic/40 bg-magic/10 px-2 py-1 text-xs text-violet-200">Explain with AI</button>
              <button onClick={() => autopsy(issue)} className="rounded border border-warning px-2 py-1 text-xs text-warning">View Autopsy</button>
              <button onClick={onPractice} className="rounded border border-accent/60 bg-accent/15 px-2 py-1 text-xs text-violet-200">Practice in Simulator</button>
            </div>
